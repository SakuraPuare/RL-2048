...
class CNNDQN(nn.Module):
    def __init__(self):
        ...
        # Input: 4x4 grid with 16 channels (one for each power of 2)
        self.conv1 = nn.Conv2d(16, 64, kernel_size=2, stride=1, padding=0)
        self.conv2 = nn.Conv2d(64, 128, kernel_size=2, stride=1, padding=0)
        
        # Fully connected layers
        self.fc1 = nn.Linear(128 * 2 * 2, 256)
        self.fc2 = nn.Linear(256, 4)  # 4 actions: up, right, down, left
        
    def forward(self, x):
        # x shape: (batch_size, 16, 4, 4)
        x = F.relu(self.conv1(x))  # (batch_size, 64, 3, 3)
...
class CNNDQNAgent:
    def __init__(self, action_size=4, buffer_size=10000, batch_size=64, gamma=0.99, 
        ...
        # Q-Networks
        self.policy_net = CNNDQN().to(device)
        self.target_net = CNNDQN().to(device)
    ...
    def encode_state(self, grid):
        """Encode the grid state for CNN input"""
        # Create a 16-channel 4x4 grid (one channel for each power of 2)
        ...
        for i in range(4):
            for j in range(4):
                val = grid[i, j]
                if val > 0:
                    # Convert value to channel index (2 -> 1, 4 -> 2, 8 -> 3, etc.)
                    channel = int(np.log2(val))
                    if channel < 16:  # Ensure we don't exceed our encoding size
                        encoded[channel, i, j] = 1
                else:
                    # Empty cell goes to channel 0
                    encoded[0, i, j] = 1
...
def calculate_reward(prev_score, current_score, prev_max_tile, current_max_tile):
    """Calculate reward based on score increase and new max tile"""
    score_reward = current_score - prev_score
    
    # Additional reward for reaching new max tile
    tile_reward = 0
    if current_max_tile > prev_max_tile:
        # Reward is proportional to the log2 of the new max tile
        tile_reward = np.log2(current_max_tile) * 10
...
def train_agent(episodes=1000, target_update=10, save_interval=100, model_path="models/cnn_rl_model.pth"):
    ...
    for episode in progress_bar:
        ...
        while not game.is_game_over():
            ...
            # Get current score and max tile before move
            prev_score = game.score
            prev_max_tile = get_max_tile(game.grid)
            
            # Take action
            game.move(action)
            
            # Calculate reward
            current_max_tile = get_max_tile(game.grid)
            reward = calculate_reward(prev_score, game.score, prev_max_tile, current_max_tile)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description='2048 CNN RL Agent')
